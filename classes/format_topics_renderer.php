 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_yinyang_format_topics_renderer extends format_topics_renderer {
    ...
    protected function start_section_list() {
        ...
        if (!$this->page->user_is_editing() && $course->coursedisplay == COURSE_DISPLAY_MULTIPAGE) {
            return html_writer::start_tag('ul', ['class' => 'topics singlecardtopics']);
        }
    ...
    protected function get_nav_links($course, $sections, $sectionno) {
        ...
        while ($back > 0 and empty($links['previous'])) {
            if ($canviewhidden || $sections[$back]->uservisible) {
                ...
                $previouslink .= "<span class='text'><span class='nav_guide'>".get_string('prev_topic', 'theme_yinyang')."</span>";
        ...
        while ($forward <= $numsections and empty($links['next'])) {
            if ($canviewhidden || $sections[$forward]->uservisible) {
                ...
                $nextlink = "<span class='text'><span class='nav_guide'>".get_string('next_topic', 'theme_yinyang')."</span><br>";
    ...
    protected function section_summary($section, $course, $mods) {
        $classattr = 'section main section-summary clearfix singlecardprogress';
        $linkclasses = '';
        ...
        if ($section->uservisible) {
            $titlewithlink = html_writer::tag('a', $title,
                array('href' => course_get_url($course, $section->section), 'class' => $linkclasses));
        }

        $progress = $this->get_section_progress($section, $course, null);

        $context = context_course::instance($course->id);
        $availabilitymessage = $this->section_availability_message($section,
        ...
        return $this->render_from_template('theme_yinyang/topics_singlecardprogress', $data);
    }
     ...
     * @return string HTML to output.
     */
    protected function get_section_progress($section, $course, $mods = null) {
        $modinfo = get_fast_modinfo($course);
