...
/**
 * Custom yinyang extras functions
 *
...
class extras {
    ...
    public static function get_course_summary_image($course, $courselink) {
        ...
        if (empty($contentimage)) {
            $url = $CFG->wwwroot . "/theme/yinyang/pix/default_course.svg";
    ...
    public static function get_mypublic_headerbuttons($context, $user) {
        ...
        if (!empty($CFG->messaging) && $USER->id != $user->id && has_capability('moodle/site:sendmessage', $context)) {
            ...
            $headerbuttons = [
                ...
                [
                    'title' => get_string($contacttitle, 'theme_yinyang'),
    ...
    /**
     * Returns the frontpage numbers info.
     *
     * @return mixed
     *
     * @throws \dml_exception
     */
    public static function get_frontpage_numbers() {
        global $DB;

        $data['users'] = $DB->count_records('user', array('deleted' => 0, 'suspended' => 0)) - 1;
        $data['courses'] = $DB->count_records('course', array('visible' => 1)) - 1;
